...
class TestKeywordsParser:
    ...
    def test_invalid_keywords_output_raises_errors(self, parser, invalid_input):
        with pytest.raises(ValueError):
            parser.parse(invalid_input)

        assert not parser.success

    def test_valid_output_returns_keywords(self, parser):
        ...
        assert parser.parse(output).get_value() == expected_keywords
